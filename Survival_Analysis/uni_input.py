    ...
    if censoring_type != 'Interval':
        ...
        if target in df.columns:
            ...
            try:
                ...
                try:
                    print('Boolean target')
                    df[target] = df[target].replace({False: 0, True: 1})
                except:
                    le = LabelEncoder()
                    df[target] = le.fit_transform(df[target])
            # elif df[target].dtype == bool or df[target].dtype == 'bool':
            #     print('Boolean target')
            #     df[target] = df[target].replace({False: 0, True: 1})
...
def lead_input_uni(orig, censor, dur, target, dur_cols):
...
def lead_input_target_uni(orig, censor, dur, target, dur_cols):
    # df = df.drop([target], axis=1)
    path = input("Enter the path of the dataset: ")
    try:
        df, _ = importFile(path)
