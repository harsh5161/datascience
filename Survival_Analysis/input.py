...
def userInputs():
        ...
        # 2 time events column can be present to indicate the start and end of events
            ...
            # datetime_to_duration method has a glitch, therefore manual conversion
                if indicator_2 in df.columns and indicator_1 in df.columns:
                    ...
                    df['duration_new'] = (df[indicator_2] - df[indicator_1]).dt.days
    ...
    if cols_to_drop[0] not in ["None", 'none', 'NONE']:
    ...
    # if censoring_type != 'INTERVAl' and len(dur_cols) == 2:
    #     if df[dur_cols[0]].dtype not in ['float', 'int', np.float32, np.float64, np.int32, np.int64, int, float]:
    #         scaler = MinMaxScaler(feature_size=(1, 100))
    #         df['duration_new'] = scaler.fit_transform(df['duration_new'])

# target is the target column name
    return df, target, dur, censoring_type, orig, cols_to_drop, dur_cols, encoder


def lead_input(orig, censor, dur, target, cols_to_drop, dur_cols, encoder):
    path = input("Enter the path of the dataset: ")
    try:
        df, _ = importFile(path)
        # display(df.head(5))
    ...
    if censor not in ['RIGHT', 'UNCENSORED', 'LEFT', 'INTERVAL']:
        print('Invalid censoring type')

    # Censoring except Interval censoring
    elif censor in ['UNCENSORED', 'RIGHT', 'LEFT']:
    ...
    try:
        ...
        for col in enc_df:
            # encoder = ce.TargetEncoder(cols=col)
            enc_df[col] = encoder.transform(enc_df[col], orig[target])
        df = pd.concat([df, enc_df], axis=1)
        print('Merged!')
