...
def formatter(x):
    try:
        try:
            ...
            return x.astype(str).strip('()%$€£¥+').lower()
    except:
        return x
...
def numeric_engineering(df):
    ...
    def returnMoney(series):
        # Remove Commas from currencies
        try:
            # try:
            for sep in [',', '$', '€', '£', '¥']:
                series = series.str.replace(sep, '')
                try:
...
def findReviewColumns(df):  # input main dataframe
    ...
    for col in sf.columns:
        ...
        # print("Length of token entities",token_len)         #create two lists that hold the value of
        # actual token entities and matched token entities respectively
        if token_len > 0:
            for ent in tokens.ents:
                # matching is done on the basis of whether the entity label is
                if (ent.label_ == 'GPE') or (ent.label_ == 'PERSON'):
                    # countries, cities, state, person (includes fictional), nationalities, religious groups,
                    # buildings, airports, highways, bridges, companies, agencies, institutes, DATE etc.
                    entity_list.append(ent.text)

            entity_counter = Counter(
                entity_list).elements()  # counts the match
            counter_length = sum(1 for x in entity_counter)
            # print("Length of matched entities",counter_length) #if there is at least a 50% match,
            # we drop that column TLDR works better on large corpus
...
def topicWordVisualiser(analyticsFrame, topicColumn, topicClass=None, topicNumber=None):
    ...
    for key in resultDict:
        ...
        # Uncomment the above two lines are run messy8.csv to understand how the output looks,
        # but implement these plots in the front end using plotly so that it looks better.
        # It goes into the new text analytics tab, users will have an option to select the topic.
        # Extract resultDict after this for loop runs so that there aren't any empty dictionaries,
        # then you can use the length of resultDict as the different topics that is present.
        plt.show()
        i += 1
...
def checkFormat(x):
    try:
        if (decimal.Decimal(str(x)).as_tuple().exponent <= -3) and (-180.0 < x < 180.0):
            return True
        else:
            return False
    except:
...
def date_engineering(df, possible_datecols, DateTime=None, validation=False):
    ...
    if not validation:  # dropping columns with missing greater than 35% only in training not scoring
...
def Segregation(df, y):
    ...
    def func(column):
        ...
        if len(minor) > 0:
            ...
            if (column.nunique() - len(minor)) in range(1, 11):
