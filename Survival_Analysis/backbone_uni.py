...
def INIT():
    try:
        df, target, dur, censoring_type, orig, dur_cols = userInputs_uni()
    except:
        return None
    ...
    if censoring_type != 'INTERVAL':
        ...
        if winnerModel is not None:
            ...
            if ll == 'Y':
                ...
                if tt_in == 'Y':
                    df_, dur_, censor_, target_ = lead_input_target_uni(orig, censoring_type, dur, target, dur_cols)
                    winnerModelTrainer_uni(df_, target_, winnerModel, dur_, censor_, instance_, orig, mode='scoring',
                                           target_availability='tt_in')
                else:
                    df_, dur_, censor_, target_ = lead_input_uni(orig, censoring_type, dur, target, dur_cols)
                    winnerModelTrainer_uni(df_, target_, winnerModel, dur_, censor_, instance_, orig, mode='scoring',
                                           target_availability=None)
