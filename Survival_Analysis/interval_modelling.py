...
class Uni_interval:
    ...
    def get_pred(self, model, lower_bound_t, upper_bound_t, lower_bound_tt,
        ...
        surv = model.survival_function_
        hazard = model.hazard_
        return lower_pred, upper_pred, surv, hazard, model
    ...
    def exponential(self, winner=False):
        ...
        lower_pred, upper_pred, surv, hazard, model = self.get_pred(model, lower_bound_t, upper_bound_t,
                                                                    lower_bound_tt, upper_bound_tt, E_train)
        self.predictionsDict['ExponentialFitter'] = [lower_pred, upper_pred, surv, hazard]
    ...
    def logLogistic(self, winner=False):
        ...
        lower_pred, upper_pred, surv, hazard, model = self.get_pred(model, lower_bound_t, upper_bound_t,
                                                                    lower_bound_tt, upper_bound_tt, E_train)
        self.predictionsDict['LogLogisticFitter'] = [lower_pred, upper_pred, surv, hazard]
    ...
    def logNormal(self, winner=False):
        ...
        lower_pred, upper_pred, surv, hazard, model = self.get_pred(model, lower_bound_t, upper_bound_t,
                                                                    lower_bound_tt, upper_bound_tt, E_train)
        self.predictionsDict['LogNormalFitter'] = [lower_pred, upper_pred, surv, hazard]
    ...
    def generalizedGamma(self, winner=False):
        ...
        lower_pred, upper_pred, surv, hazard, model = self.get_pred(model, lower_bound_t, upper_bound_t,
                                                                    lower_bound_tt, upper_bound_tt, E_train)
        self.predictionsDict['GeneralizedGammaFitter'] = [lower_pred, upper_pred, surv, hazard]
    ...
    def weibull(self, winner=False):
        ...
        lower_pred, upper_pred, surv, hazard, model = self.get_pred(model, lower_bound_t, upper_bound_t,
                                                                    lower_bound_tt, upper_bound_tt, E_train)
        self.predictionsDict['WeibullFitter'] = [lower_pred, upper_pred, surv, hazard]
    ...
    def get_lead(self, model_instance):
        lower_ = self.lower_bound_tt
        upper = self.upper_bound_tt
        print('...Running Scoring...')
        with joblib.parallel_backend('dask'):
            upper_pred = model_instance.predict(upper)
            lower_pred = model_instance.predict(lower_)
            surv = model_instance.survival_function_
            hazard = model_instance.hazard_

            return [lower_pred, upper_pred, surv, hazard]

    def get_lead_sc(self, model_instance, up, lower):
        upper_pred = model_instance.predict(up)
        lower_pred = model_instance.predict(lower)
        surv = model_instance.survival_function_
        hazard = model_instance.hazard_
...
class Multi_interval:
    ...
    def weibullAFT(self, winner=False):
        ...
        self.predictionsDict['WeibullAFTFitter'] = [survival, hazard]
    ...
    def logNormalAFT(self, winner=False):
        ...
        survival = model.predict_survival_function(X_test)
        hazard = model.predict_hazard(X_test)
        self.predictionsDict['LogNormalAFTFitter'] = [survival, hazard]
    ...
    def generalizedGammaAFT(self, winner=False):
        ...
        cum_hazard = model.predict_cumulative_hazard(X_test)
        survival = model.predict_survival_function(X_test)
        self.predictionsDict['GeneralizedGammaRegressionFitter'] = [survival]
