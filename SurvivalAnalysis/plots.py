...
def plot_pysurv_cindex(resultsDict):
    df = pd.DataFrame(resultsDict, index=["concordance_index"])
    colors = plt.rcParams["axes.prop_cycle"].by_key()["color"]
    pallette = plt.cm.get_cmap("tab20c", len(df.columns))
    colors = [pallette(x) for x in range(len(df.columns))]
    color_dict = dict(zip(df.columns, colors))
    ...
    # C-Index plot
    df.loc["concordance_index"].sort_values().plot(
        kind="bar", colormap="Paired", color=[
            color_dict.get(
                x, "#333333") for x in df.loc["concordance_index"].sort_values().index], )
    plt.legend()
    plt.title("Concordance Index, higher is better")
...
def plot_aic(resultsDict):
    ...
    plt.legend()
    plt.title("AIC score, higher is better")
...
def plot_cdf(modelsDict):
    df = pd.DataFrame(modelsDict, index=['models'])
    df = df.drop(['GeneralizedGammaFitter', 'ExponentialFitter'], axis=1)
    colors = plt.rcParams["axes.prop_cycle"].by_key()["color"]
    pallette = plt.cm.get_cmap("tab20c", len(df.columns))
    colors = [pallette(x) for x in range(len(df.columns))]
    color_dict = dict(zip(df.columns, colors))
...
def pysurv_winner(predictionsDict, result):
    ...
    plt.plot(predictionsDict[result][4])
    plt.show()
    fig = plt.figure(figsize=(10, 10))
    cmap = get_cmap(50)
    for i in [1, 2]:
