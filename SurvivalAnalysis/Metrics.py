...
def _concordance_index(time_events, predicted, actual):
    ...
    :param actual: a length-n iterable censoring flags, 1 if observed, 0 if not.
    :param predicted:  a length-n iterable of predicted scores - these could be survival times, or hazards, etc.
...
def _cindex_interval(time_events, predicted, actual):
    predicted = predicted.to_frame()
    predicted = predicted.drop_duplicates(keep='first')
    predicted.reset_index(inplace=True)
    time_events = pd.DataFrame(time_events)
    final = pd.concat([time_events, predicted], axis=1)
    final.dropna(inplace=True)
    c_score = utils.concordance_index(final.iloc[:, 0], final.iloc[:, -1])
    if c_score < 0:
        c_score = utils.concordance_index(final.iloc[:, 0], -1 * final.iloc[:, -1])
        return c_score
    else:
        return c_score
...
def pysurv_cindex(model, X, T, E):
    """ Concordance index score for PySurvival models """
    return concordance_index(model, X, T, E)
...
surv_metric1 = {'_concordance_index': _concordance_index,
                '_cindex_interval': _cindex_interval}
surv_metric2 = {'simple_cindex': simple_cindex}
surv_metric3 = {'aic': aic}
pysurv_metric = {'concordance_index': pysurv_cindex}
...
def evaluate_1(time_events, predicted, actual):
    results = {'_concordance_index': surv_metric1['_concordance_index'](time_events, predicted, actual)}
    return results


def evaluate_1_int(time_events, predicted, actual):
    results = {'_cindex_interval': surv_metric1['_cindex_interval'](time_events, predicted, actual)}
    return results


def pysurv_eval(model, X, T, E):
    results = {'concordance_index': pysurv_metric['concordance_index'](model, X, T, E)}
    return results
