...
#     date_cols = getDateColumns(X.sample(1500) if len(X) > 1500 else X)  # old logic
    ...
    concat_list = [num_df,disc_df,DATE_DF,TEXT_DF,EMAIL_DF,URL_DF]
    ...
    init_info = {'NumericColumns':num_df.columns,'NumericMean':num_df.mean().to_dict(),'DiscreteColumns':disc_df.columns,
                ...
                'some_list':some_list,'remove_list':remove_list,'lda_models':lda_models,'email_cols':email_cols,'url_cols':url_cols,'EMAIL_STATUS':EMAIL_STATUS,'URL_STATUS':URL_STATUS}
    print(' #### DONE ####')
